...
is_form_associated_element :: proc(node: ^Node) -> bool {
    ...
    if !is_el do return false 
	switch el.local_name {
	case "button", "fieldset", "input", "object", "output", "select", "textarea", "img": return true
	}
...
doc_type_to_string :: proc(dt: Document_Type) -> string {
    switch dt {
    case .DT_XML: return "xml"
    case .DT_HTML: return "html" 
    } 
    return ""
}
...
doc_mode_to_string :: proc(dt: Document_Mode) -> string {
    switch dt {
    case .No_Quirks: return "no-quirks"
    case .Quirks: return "quirks" 
    case .Limited_Quirks: return "limited-quirks"
    } 
    return ""
}
